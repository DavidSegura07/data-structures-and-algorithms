...
let peopleWithExclamation = addExclamation(people);
console.log(peopleWithExclamation);
/* The code is defining an array called `people` with four string elements. It then defines a function
called `addExclamation` that takes an array as an argument. Inside the function, it creates an empty
array called `newArr`. It uses the `forEach` method to iterate over each element in the input array.
For each element, it concatenates an exclamation mark to the end of the string and stores it in a
variable called `newPerson`. It then pushes `newPerson` into the `newArr` array. Finally, it returns
the `newArr` array. */
...
const myArray = [];
const upperCaseArray = allUpperCase(myArray);
...
const speaker = (words, callback) => {
  let greetingPerson = [];
  words.forEach(word => {
    greetingPerson.push(callback(word));
  });
  return greetingPerson;
};

const greetingArray =[];
const loudGreetings = speaker(greetingArray, greeting);
console.log(loudGreetings);
